...
function makeButton(label,className,onClick){const btn=make("button",className,label);btn.type="button";btn.addEventListener("click",onClick);return btn;}
...
function dateKey(value){if(!value)return "";const s=String(value);const match=s.match(/^(\d{4}-\d{2}-\d{2})/);if(match)return match[1];const d=new Date(value);return Number.isNaN(d.getTime())?"":localDateKey(d);}
function formatDate(value){const key=dateKey(value);if(!key)return "일정 없음";const [,m,d]=key.split("-");return `${Number(m)}.${Number(d)}`;}
function dDay(value){const key=dateKey(value);if(!key)return "";const now=new Date(`${localDateKey()}T00:00:00`),target=new Date(`${key}T00:00:00`);const diff=Math.round((target-now)/86400000);if(diff===0)return "오늘";return diff>0?`D-${diff}`:`D+${Math.abs(diff)}`;}
...
function stateSignature(){return [state.status,state.petId,state.loadedAt,state.pets.length,state.error].join("|");}
...
async function petApi(action,{method="GET",body,params={}}={}){
  ...
  const response=await fetch(url,options),payload=await response.json().catch(()=>({}));if(!response.ok){const err=new Error(payload.error||"PetLife 정보를 불러오지 못했어요.");err.status=response.status;throw err;}return payload;
}
...
function openPetLife(tab="timeline",category=""){
  const openAlready=!!document.querySelector("#petlife-react-root .pl-backdrop"),launcher=document.querySelector("#petlife-react-root .pl-launcher");if(!openAlready){if(launcher)launcher.click();else{setTimeout(()=>openPetLife(tab,category),120);return;}}
  setTimeout(()=>{clickMatching("#petlife-react-root .pl-tabs button",TAB_LABELS[tab]||TAB_LABELS.timeline);if(category)setTimeout(()=>clickMatching("#petlife-react-root .pl-quicktypes button",[TYPE_LABELS[category]||category]),140);},120);
}
function selectedPet(){return state.pets.find(p=>p.id===state.petId)||state.pets[0]||null;}
function summary(){
  const pet=selectedPet(),detail=state.detail;if(!pet||!detail)return null;const today=localDateKey(),walks=detail.entries.filter(e=>e.category==="walk"&&dateKey(e.occurredOn)===today),walkMinutes=walks.reduce((sum,e)=>sum+(Number(e.durationMinutes)||0),0);const upcoming=detail.upcoming||[],nextVaccine=upcoming.find(e=>e.category==="vaccine")||upcoming[0]||null;const report=detail.report||{},weights=Array.isArray(report.weights)?report.weights:[],recentWeight=weights.length?weights[weights.length-1].kg:null,weight=pet.weightKg??recentWeight;const photos=Array.isArray(detail.album?.photos)?detail.album.photos:[],recentPhoto=photos.length?photos[photos.length-1]:null,photoUrl=safePhoto(recentPhoto?.photoUrl)||safePhoto(pet.photoUrl);return {pet,walks,walkMinutes,nextVaccine,report,weight,photoUrl,recentPhoto};
}
function statCard(icon,label,value,sub,actionLabel,onAction,extraClass=""){const card=make("article",`pgh-stat ${extraClass}`.trim()),top=make("div","pgh-stat-top");top.append(make("span","pgh-stat-icon",icon),make("small","",label));card.append(top,make("strong","",value),make("p","",sub));if(actionLabel)card.append(makeButton(actionLabel,"pgh-linkbtn",onAction));return card;}
function renderLoading(root,title="오늘의 우리 아이 관리"){root.replaceChildren();const shell=make("div","pgh-loading");shell.append(make("div","pgh-loading-mark","🐾"),make("div","",[make("b","",title),make("span","","PetLife 기록을 불러오는 중…")]));root.append(shell);}
function renderEmpty(root){root.replaceChildren();const wrap=make("div","pgh-empty");wrap.append(make("span","pgh-empty-icon","🐾"),make("div","pgh-empty-copy",[make("small","","PETLIFE · START"),make("h2","","우리 아이의 평생기록을 시작해 보세요"),make("p","","한 번 등록하면 체중, 접종, 병원, 산책, 사진과 건강기록이 하나의 시간축으로 이어집니다.")]),makeButton("우리 아이 등록","pgh-primary",()=>openPetLife("timeline")));root.append(wrap);}
...
function renderHome(root){
  if(state.status==="loading"||state.status==="idle"){renderLoading(root);return;}if(state.status==="error"){renderError(root);return;}if(!state.pets.length){renderEmpty(root);return;}const s=summary();if(!s){renderLoading(root);return;}root.replaceChildren();const shell=make("div","pgh-shell"),head=make("header","pgh-head"),identity=make("div","pgh-identity");
  ...
  const copy=make("div","pgh-headcopy");copy.append(make("small","","PETLIFE · TODAY"),make("h2","",`${s.pet.name} 오늘 관리`),make("p","","오늘 챙길 것과 최근 기록을 한눈에 확인하세요."));identity.append(copy);head.append(identity);const headActions=make("div","pgh-head-actions");
  if(state.pets.length>1){const label=make("label","pgh-pet-switch");label.append(make("span","","아이 선택"));const select=make("select");state.pets.forEach(p=>{const option=make("option","",p.name);option.value=p.id;option.selected=p.id===s.pet.id;select.append(option);});select.addEventListener("change",e=>selectPet(e.target.value));label.append(select);headActions.append(label);}headActions.append(makeButton("PetLife 전체보기","pgh-primary",()=>openPetLife("timeline")));head.append(headActions);shell.append(head);
  const grid=make("div","pgh-stat-grid"),delta=formatDelta(s.report?.weightDelta);grid.append(statCard("⚖️","현재 체중",formatWeight(s.weight),delta,"몸무게 기록",()=>openPetLife("timeline","weight"),"pgh-weight"),statCard("💉",s.nextVaccine?.category==="vaccine"?"다음 예방접종":"다음 건강일정",s.nextVaccine?dDay(s.nextVaccine.nextDueOn):"일정 없음",s.nextVaccine?`${s.nextVaccine.title} · ${formatDate(s.nextVaccine.nextDueOn)}`:"접종·병원·약 일정을 등록해 보세요.","일정 보기",()=>openPetLife("schedule"),"pgh-schedule"),statCard("🐕","오늘 산책",s.walks.length?`${s.walks.length}회 · ${s.walkMinutes}분`:"아직 기록 없음",s.walks.length?"오늘의 산책이 기록됐어요.":"산책 후 바로 남기면 생활패턴이 보여요.","산책 기록",()=>openPetLife("timeline","walk"),"pgh-walk"));
  const photo=make("article","pgh-stat pgh-photo"),photoTop=make("div","pgh-stat-top");photoTop.append(make("span","pgh-stat-icon","📷"),make("small","","최근 사진"));photo.append(photoTop);if(s.photoUrl){const img=make("img","pgh-recent-photo");img.src=s.photoUrl;img.alt=`${s.pet.name} 최근 사진`;photo.append(img,make("p","",s.recentPhoto?`${formatDate(s.recentPhoto.occurredOn)} 기록`:"프로필 사진"));}else photo.append(make("div","pgh-photo-empty","첫 사진을 남겨보세요"),make("p","","사진이 쌓이면 성장앨범이 자동으로 정리돼요."));photo.append(makeButton("성장앨범","pgh-linkbtn",()=>openPetLife("album")));grid.append(photo);shell.append(grid);
  const insight=make("div","pgh-report-preview"),insightCopy=make("div","");insightCopy.append(make("small","","30-DAY CARE REPORT"),make("b","",`이번 달 ${s.pet.name} 관리 리포트`),make("p","",s.report?.insights?.[0]||"기록이 쌓이면 체중 변화와 생활패턴 관리 포인트를 보여드려요."));insight.append(make("span","pgh-report-icon","📊"),insightCopy,makeButton("리포트 보기","pgh-secondary",()=>openPetLife("report")));shell.append(insight);
...
function findMyPetHeading(){const candidates=[...document.querySelectorAll("#root h1,#root h2,#root h3")];return candidates.find(node=>{if(!isVisible(node)||node.closest(".petgrow-dashboard-home,#petlife-react-root,#pg-petlife-home-dashboard,#pg-petlife-my-pet-bridge"))return false;const t=cleanText(node).replace(/\s+/g,"").toLowerCase();return (t.includes("우리아이")&&t.length<=16)||(t.includes("mypet")&&t.length<=16);})||null;}
function ensureHome(){
  const home=homeElement();if(!home)return false;let root=home.querySelector(`#${HOME_ID}`);if(state.status==="unauth"){root?.remove();return true;}if(!root){root=make("section","pgh-home-dashboard");root.id=HOME_ID;root.setAttribute("aria-label","오늘의 우리 아이 관리");const quick=home.querySelector(".dash-quick-grid")?.closest(".dash-section");if(quick)home.insertBefore(root,quick);else home.prepend(root);}const signature=stateSignature();if(root.dataset.pgSignature!==signature){root.dataset.pgSignature=signature;renderHome(root);}return true;
}
function renderMyPet(root){
  if(state.status==="loading"||state.status==="idle"){root.replaceChildren(make("span","pgmypet-icon","🐾"),make("div","pgmypet-copy",[make("b","","PetLife 평생기록"),make("p","","우리 아이의 기록을 연결하는 중…")]));return;}root.replaceChildren();const s=summary(),icon=make("span","pgmypet-icon","🐾"),copy=make("div","pgmypet-copy");copy.append(make("small","","PETLIFE"),make("b","","평생기록"),make("p","",s?`${s.pet.name}의 체중 · 접종 · 병원 · 산책 · 사진을 이 프로필과 함께 관리해요.`:"우리 아이를 등록하면 성장과 건강 관리 기록이 한곳에 이어집니다."));const actions=make("div","pgmypet-actions");actions.append(makeButton(s?"기록 보기":"PetLife 시작","pgh-primary",()=>openPetLife("timeline")));if(s)actions.append(makeButton("일정","pgh-secondary",()=>openPetLife("schedule")),makeButton("리포트","pgh-secondary",()=>openPetLife("report")));root.append(icon,copy,actions);
}
