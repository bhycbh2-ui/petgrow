...
function setButtonsBusy(){
  ...
  const backup=box.querySelector("#pg-run-backup"),verify=box.querySelector("#pg-verify-backup");
  if(backup){backup.disabled=backupRunning||verifyRunning||backup.dataset.enabled!=="1";backup.textContent=backupRunning?"백업 실행 중…":"지금 백업";}
  if(verify){verify.disabled=backupRunning||verifyRunning||verify.dataset.enabled!=="1";verify.textContent=verifyRunning?"검증 중…":"무결성 검증";}
}
async function runBackup(){
  if(backupRunning||verifyRunning)return;
  ...
  try{
    ...
    lastLoad=0;window.setTimeout(()=>refresh(true),800);
  }catch(e){
...
async function verifyBackup(){
  ...
  try{
    const result=await requestJson("/api/admin-backup-verify",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"});
    if(!result)return;
...
async function refresh(force=false){
  ...
  try{
    ...
    if(!overview||!health){box.remove();return;}
