/* PetGrow saved pet-state recovery — 2026-08-18 */
(() => {
  ...
  /* Retry only saved-pet state GETs. This file does not alter navigation or Tarot DOM. */
  if (!window.__pgPetStateFetchRetryV1) {
    ...
    window.fetch = async function(input, init = {}) {
      ...
      if (method !== 'GET' || !STATE_KEYS.has(stateKey)) return nativeFetch(input, init);
        ...
        try {
          ...
          if (response.ok) {
            try {
              ...
              if (json && Object.prototype.hasOwnProperty.call(json, 'value') && json.value != null) return response;
            } catch {
              return response;
            }
          }
  ...
  function petButtons(page) {
    return [...page.querySelectorAll(':scope > .feature-pet-picker button, .feature-pet-picker button')].filter((button) => text(button));
  }
