...
function PetPointDashboard({compact=false}){
  ...
  useEffect(()=>{load();const h=e=>{const ev=e.detail||{};setToast(ev);setD(v=>v?{...v,balance:ev.balance??v.balance}:v);clearTimeout(toastTimer.current);toastTimer.current=setTimeout(()=>setToast(null),3000);setTimeout(()=>load(true),180)};window.addEventListener("petgrow:points",h);const poll=setInterval(()=>load(true),5000);return()=>{window.removeEventListener("petgrow:points",h);clearInterval(poll);clearTimeout(toastTimer.current)}},[]);
