...
function HamburgerMenu({ open, onClose, view, onNavigate, onOpenAccount, account }) {
  ...
  const items = [
    ...
    { key: "nearby", label: t.nearbyNav, Icon: MapPinIcon },
...
@media(max-width:760px){.support-head{align-items:center}.support-head h1{font-size:22px}.support-back-right{padding:8px 12px}.petmusic-page{padding:2px 12px 76px}.petmusic-top5{grid-template-columns:repeat(5,132px);overflow-x:auto;padding-bottom:5px}.petmusic-grid{grid-template-columns:1fr}.petmusic-card{grid-template-columns:104px minmax(0,1fr)}.petmusic-cover{width:104px;height:104px}.admin-music-form{grid-template-columns:1fr}.admin-music-form .full{grid-column:auto}.admin-music-row{grid-template-columns:46px minmax(0,1fr)}.admin-music-actions{grid-column:1/-1;justify-content:flex-start}}
  ...
  *,*::before,*::after{
  ...
  @media(max-width:700px){.nearby-page{padding:12px 12px 36px}.nearby-hero{padding:18px;display:grid}.nearby-hero h1{font-size:25px}.nearby-locate-btn{width:100%;justify-content:center}.nearby-map{height:330px}.nearby-place{grid-template-columns:auto 1fr}.nearby-place-actions{grid-column:2;justify-content:flex-start}.nearby-place-title{display:block}.nearby-place-title strong{display:block;margin-top:3px}.nearby-review-head{display:grid}.nearby-review-head .bg-btn{width:100%}.nearby-review-compose-foot{align-items:flex-start;flex-direction:column}.nearby-review-compose-foot .bg-btn{width:100%}.nearby-review-meta small{margin-left:0;width:100%}.admin-menu-row{grid-template-columns:82px 1fr 60px}}
    ...
    .dash-section{margin-top:22px}.dash-section-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}.dash-section-head h2{font-size:16px;margin:0}.dash-section-head span{font-size:10px;color:#9AA39D}.dash-quick-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px}.dash-quick-grid button{border:1px solid #E1E9E2;background:#fff;border-radius:20px;min-height:96px;padding:13px 8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;font-family:inherit;font-weight:800;color:#38433B;cursor:pointer;box-shadow:0 6px 18px rgba(34,48,39,.035);transition:.16s}.dash-quick-grid button:hover{transform:translateY(-2px);border-color:#CFE0D2}.dash-quick-grid i{width:43px;height:43px;border-radius:15px;background:#F0F6F0;display:grid;place-items:center;font-style:normal;font-size:20px}.dash-quick-grid span{font-size:11px}
  ...
  return (
    <div className="legal-page-shell petgrow-dashboard-home">
      <section className="dash-welcome">
        <div><span className="dash-eyebrow">PetGrow</span><h1>{account?.name ? t.homeGreeting(account.name) : (lang === "en" ? "Welcome to PetGrow! 🐾" : "오늘도 우리 아이와 행복한 하루 🐾")}</h1><p>{lang === "en" ? "Everything your pet needs, in one simple dashboard." : "우리 아이의 성장·음악·주변 시설·커뮤니티를 한곳에서 확인해요."}</p></div>
      ...
      <section className={`dash-pet-spotlight ${pet ? "has-pet" : "empty"}`} onClick={onGoPets}>
        {pet ? <>
          <div className="dash-pet-photo">{pet.profile.profileImage ? <img src={pet.profile.profileImage} alt={`${pet.profile.name || "반려동물"} 프로필`} fetchPriority="high" /> : <span>{pet.species === "cat" ? "🐱" : "🐶"}</span>}</div>
          <div className="dash-pet-copy"><small>{lang === "en" ? "TODAY WITH MY PET" : "오늘의 우리 아이"}</small><h2>{pet.profile.name || (lang === "en" ? "My Pet" : "우리 아이")}</h2><p>{[pet.profile.breedName, petAgeLabel(pet.profile.birthDate, lang)].filter(Boolean).join(" · ")}</p><div className="dash-pet-metrics"><span><b>{weight ? `${Number(weight).toFixed(1)}kg` : "—"}</b><small>{lang === "en" ? "Weight" : "현재 체중"}</small></span><span><b>{pet.profile.gender === "male" ? "♂" : pet.profile.gender === "female" ? "♀" : "—"}</b><small>{lang === "en" ? "Gender" : "성별"}</small></span><span><b>{pet.species === "cat" ? "CAT" : "DOG"}</b><small>{lang === "en" ? "Type" : "구분"}</small></span></div></div>
      ...
      <section className="dash-section"><div className="dash-section-head"><h2>{lang === "en" ? "Quick access" : "자주 사용하는 메뉴"}</h2><span>{lang === "en" ? "Tap to open" : "빠르게 이동"}</span></div><div className="dash-quick-grid">{quick.map(([key,icon,label])=><button type="button" key={key} onClick={()=>key==="pets"?onGoPets():key==="more"?onGoView("more"):onGoView(key)}><i>{icon}</i><span>{label}</span></button>)}</div></section>
 ...
 return <div className="admin-reports-page">
  ...
  <button className="bg-btn bg-btn-ghost admin-exit-btn" onClick={onBack}>← 회원정보</button>
   ...
   {tab==="dashboard"&&<>
     ...
     <div className="bg-card"><h3 style={{marginTop:0}}>📱 접속 환경 통계</h3><p className="bg-sub" style={{fontSize:12}}>웹사이트 · 모바일 웹/PWA · 앱 이용을 익명 세션 기준으로 구분해요.</p>{(stats?.platformUsage||[]).length?<div className="admin-platform-grid">{stats.platformUsage.map(x=><div className="admin-platform-card" key={x.platform}><b>{{web:"🖥️ 웹사이트",mobile_web:"📱 모바일 웹",pwa:"📲 PWA",android:"🤖 Android 앱",ios:"🍎 iOS 앱"}[x.platform]||x.platform}</b><strong>{Number(x.d30)||0}</strong><small>30일 · 7일 {Number(x.d7)||0} · 오늘 {Number(x.today)||0}</small></div>)}</div>:<div className="bg-sub" style={{padding:"10px 0"}}>접속 환경 통계가 아직 없어요.</div>}</div>
   </>}
   {tab==="reporting"&&<div className="admin-reporting-page">
     <div className="bg-card admin-reporting-head"><div><h2>📊 운영 보고서</h2><p className="bg-sub">전날까지의 데이터를 일일·주간·월간 단위로 보기 쉽게 정리해요.</p></div><div className="admin-reporting-tabs">{[["daily","일일보고"],["weekly","주간보고"],["monthly","월간보고"]].map(([k,l])=><button className={reportPeriod===k?"active":""} key={k} onClick={()=>loadReport(k)}>{l}</button>)}</div></div>
     {!reportData&&!reportLoading?<div className="bg-card admin-report-empty"><b>보고서 종류를 선택해 주세요.</b><p className="bg-sub">일일보고는 어제, 주간보고는 최근 7일(오늘 제외), 월간보고는 지난달 기준이에요.</p><button className="bg-btn" onClick={()=>loadReport("daily")}>일일보고 불러오기</button></div>:reportLoading?<div className="bg-card admin-report-empty">보고서를 계산하는 중...</div>:<>{(()=>{const r=reportData?.summary||{};return <><div className="admin-report-kpi-grid">{[["방문 세션",r.sessions||0,"👥"],["신규 회원",r.newMembers||0,"✨"],["Pet톡 글",r.posts||0,"💬"],["Pet톡 댓글",r.comments||0,"🗨️"],["음악 좋아요",r.musicLikes||0,"♥"],["음악 댓글",r.musicComments||0,"🎵"],["주변 Pet 후기",r.placeReviews||0,"📍"],["신고 접수",r.reports||0,"🚨"]].map(([l,v,ic])=><div className="bg-card admin-report-kpi" key={l}><span>{ic}</span><strong>{v}</strong><small>{l}</small></div>)}</div><div className="admin-reporting-grid"><div className="bg-card"><h3>🏆 이용 메뉴 TOP</h3>{(reportData.topMenus||[]).length?(reportData.topMenus||[]).slice(0,8).map((x,i)=><div className="admin-report-rank" key={x.dimension}><b>{i+1}. {({home:"홈",about:"소개",pets:"우리 아이",nearby:"내 주변 Pet",community:"Pet톡",saju:"Pet사주",petbti:"PetBTI",music:"Pet음악",tips:"Pet정보",my:"회원정보",support:"고객지원",admin:"관리자센터"}[x.dimension]||x.dimension)}</b><span>{Number(x.count)||0}회</span></div>):<p className="bg-sub">집계 데이터가 없어요.</p>}</div><div className="bg-card"><h3>📱 접속 환경</h3>{(reportData.platforms||[]).length?(reportData.platforms||[]).map(x=><div className="admin-report-rank" key={x.platform}><b>{{web:"웹사이트",mobile_web:"모바일 웹",pwa:"PWA",android:"Android 앱",ios:"iOS 앱"}[x.platform]||x.platform}</b><span>{Number(x.count)||0}</span></div>):<p className="bg-sub">집계 데이터가 없어요.</p>}<hr/><p className="bg-sub">내 주변 Pet 후기 평균 <b>★ {r.placeAvgRating||0}</b></p></div></div></>})()}<div className="bg-card admin-report-mail"><div><h3>✉️ 메일 보고서용 요약</h3><p className="bg-sub">help.petgrow@gmail.com으로 보낼 때 그대로 사용할 수 있는 형식이에요.</p></div><textarea className="bg-input" readOnly value={reportText()} /><div className="admin-report-actions"><button onClick={()=>navigator.clipboard?.writeText(reportText()).then(()=>window.alert("보고서 내용을 복사했어요."))}>내용 복사</button><a className="bg-btn" href={`mailto:help.petgrow@gmail.com?subject=${encodeURIComponent(`[PetGrow] ${{daily:"일일",weekly:"주간",monthly:"월간"}[reportPeriod]} 운영보고`)}&body=${encodeURIComponent(reportText())}`}>메일 작성</a></div></div></>}
   </div>}
