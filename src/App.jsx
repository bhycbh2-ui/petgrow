...
function HomePage({ account, pets = [], lang, onGoPets, onGoView }) {
  ...
  return (
    <div className="legal-page-shell petgrow-dashboard-home">
      ...
      <section className="dash-section"><div className="dash-section-head"><h2>{lang === "en" ? "Quick access" : "자주 사용하는 메뉴"}</h2><button type="button" className="bg-chip" onClick={()=>setQuickEditing(v=>!v)}>{quickEditing?(lang==='en'?'Done':'완료'):(lang==='en'?'Edit':'편집')}</button></div>{quickEditing&&<div className="bg-card" style={{padding:14,marginBottom:12}}><p className="bg-sub" style={{fontSize:12,margin:'0 0 10px'}}>{lang==='en'?'Choose up to six shortcuts, then reorder them below. Signed-in choices sync to your account.':'원하는 메뉴를 최대 6개까지 선택한 뒤 아래에서 순서를 바꿀 수 있어요. 로그인하면 계정에 저장돼 다른 기기에서도 그대로 보여요.'}</p><div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:12}}>{allQuick.map(([key,icon,label])=><button type="button" key={key} className={`bg-chip ${quickKeys.includes(key)?'active':''}`} onClick={()=>toggleQuick(key)}>{icon} {label}</button>)}</div><div className="quick-order-list">{quick.map(([key,icon,label],i)=><div className="quick-order-row" key={key}><span><i>{icon}</i><b>{label}</b></span><div><button type="button" disabled={i===0} onClick={()=>moveQuick(key,'first')}>{lang==='en'?'First':'맨앞'}</button><button type="button" disabled={i===0} onClick={()=>moveQuick(key,'left')}>←</button><button type="button" disabled={i===quick.length-1} onClick={()=>moveQuick(key,'right')}>→</button><button type="button" disabled={i===quick.length-1} onClick={()=>moveQuick(key,'last')}>{lang==='en'?'Last':'맨뒤'}</button></div></div>)}</div></div>}<div className="dash-quick-grid">{quick.map(([key,icon,label])=><button type="button" key={key} onClick={()=>key==="pets"?onGoPets():onGoView(key)}><i>{icon}</i><span>{label}</span></button>)}</div></section>
