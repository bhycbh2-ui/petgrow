...
export default async function handler(req,res){
  ...
  try{
    ...
    if(musicOnly){
      const musicResult=await sql`select id,title,description,species,vocal_type,mood,cover_url,audio_url,like_count,comment_count,play_count from pg_music_tracks where active=true order by (like_count*4+comment_count*3+play_count)::numeric desc,created_at desc limit 5`;
      res.setHeader("Cache-Control","public, max-age=60, s-maxage=600, stale-while-revalidate=3600");
      return res.status(200).json({news:[],top5:mapMusic(musicResult.rows),fast:true});
    }
