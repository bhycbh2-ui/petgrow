...
export default async function handler(req,res){
  ...
  try{
    const [newsResult,musicResult]=await Promise.all([
      ...
      sql`select id,title,description,species,vocal_type,mood,cover_url,audio_url,like_count,comment_count,play_count from pg_music_tracks where active=true order by (like_count*4+comment_count*3+play_count)::numeric desc,created_at desc limit 3`
    ]);
