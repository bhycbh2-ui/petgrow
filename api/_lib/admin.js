...
export async function isAdminUserId(userId){await ensureSchema();if(!userId)return false;const {rows}=await sql`select 1 from pg_admins where user_id=${userId}`;return !!rows[0]}
export async function adminExists(){await ensureSchema();const {rows}=await sql`select exists(select 1 from pg_admins) ok`;return !!rows[0]?.ok}
