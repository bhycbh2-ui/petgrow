...
export default async function handler(req,res){
 ...
 try{
  ...
  const cap=a==="reports"||a==="restrict"||a==="unblock"||a==="resolve"||a==="place-reports"||a==="place-review-hide"||a==="place-review-resolve"||a==="music-comment-reports"||a==="music-comment-hide"||a==="music-comment-resolve"?"reports":
            a==="stats"||a==="report-summary"?"dashboard":a==="logs"?"logs":
    ...
    const queries={
      ...
      platformUsage:()=>sql`
        select platform,
          ...
          count(*) filter(where day>=(now() at time zone 'Asia/Seoul')::date-6)::int d7,
          count(*) filter(where day>=(now() at time zone 'Asia/Seoul')::date-29)::int d30
        from pg_analytics_sessions
        where day>=(now() at time zone 'Asia/Seoul')::date-29
        group by platform order by d30 desc
      `
    };
  ...
  if(a==="report-summary"&&req.method==="GET"){
    ...
    return res.status(200).json({period,start,end,summary:{sessions:sessions.rows[0]?.sessions||0,newMembers:members.rows[0]?.new_members||0,posts:community.rows[0]?.posts||0,comments:community.rows[0]?.comments||0,musicPlays:music.rows[0]?.plays||0,musicLikes:music.rows[0]?.likes||0,musicComments:music.rows[0]?.comments||0,placeReviews:places.rows[0]?.reviews||0,placeAvgRating:places.rows[0]?.avg_rating||0,reports:reportsR.rows[0]?.reports||0},topMenus:menu.rows,platforms:platform.rows});
  }
  ...
  if(a==="music-comment-reports"&&req.method==="GET"){
    const {rows}=await sql`select mr.id,mr.comment_id,mr.reason,mr.detail,mr.status,mr.created_at,c.track_id,c.content,c.user_id target_user_id,t.title track_title,coalesce(author.nickname,'PetGrow 회원') author_nickname,coalesce(reporter.nickname,'PetGrow 회원') reporter_nickname from pg_music_comment_reports mr join pg_music_comments c on c.id=mr.comment_id join pg_music_tracks t on t.id=c.track_id left join pg_users author on author.id=c.user_id left join pg_users reporter on reporter.id=mr.reporter_user_id order by case when mr.status='open' then 0 else 1 end,mr.created_at desc limit 100`;
    return res.status(200).json({reports:rows});
  }
  if(a==="music-comment-hide"&&req.method==="POST"){
    const {commentId,reportId}=req.body||{};if(!commentId)return res.status(400).json({error:"댓글 정보가 없어요."});
    await sql`update pg_music_comments set status='hidden',updated_at=now() where id=${commentId}`;
    if(reportId)await sql`update pg_music_comment_reports set status='resolved',reviewed_at=now(),reviewed_by=${u} where id=${reportId}`;
    await logAdmin(u,"MUSIC_COMMENT_HIDE",null,reportId||null,{commentId});return res.status(200).json({ok:true});
  }
  if(a==="music-comment-resolve"&&req.method==="POST"){
    const {reportId}=req.body||{};if(!reportId)return res.status(400).json({error:"신고 정보가 없어요."});
    await sql`update pg_music_comment_reports set status='resolved',reviewed_at=now(),reviewed_by=${u} where id=${reportId}`;
    await logAdmin(u,"MUSIC_COMMENT_REPORT_RESOLVED",null,reportId);return res.status(200).json({ok:true});
  }
