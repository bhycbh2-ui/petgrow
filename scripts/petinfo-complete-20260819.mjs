...
const s=app.indexOf('const TIPS_DATA = [');
const e=app.indexOf('\n];',s);
if(s<0||e<0) throw new Error('TIPS_DATA not found');
const block=app.slice(s,e);
if(!block.includes('id: "t194"')) throw new Error('Expected latest PetInfo t194 not found');
if(block.includes('id: "t195"')) { console.log('Completion items already present'); process.exit(0); }
const additions=`
...
const cats=['dog','cat','health','life','food','training','safety','grooming'];
for(const c of cats) if(parsed.filter(m=>m[2]===c).length!==1) throw new Error(`Expected 1 completion item for ${c}`);
for(const m of parsed){ if(block.includes(`id: "${m[1]}"`)) throw new Error(`Duplicate id ${m[1]}`); if(block.includes(`ko: "${m[3]}"`)) throw new Error(`Duplicate title ${m[3]}`); }
app=app.slice(0,e)+additions+app.slice(e);
const ub=app.slice(s,app.indexOf('\n];',s));
const ids=[...ub.matchAll(/id: "(t\d+)"/g)].map(m=>m[1]);
const titles=[...ub.matchAll(/title: \{ ko: "([^"]+)"/g)].map(m=>m[1]);
if(new Set(ids).size!==ids.length) throw new Error('Duplicate IDs after update');
if(new Set(titles).size!==titles.length) throw new Error('Duplicate Korean titles after update');
fs.writeFileSync(path,app);
console.log('Added 8 completion items, producing 2 new items per category for 2026-08-19 when combined with the 8-item partial main update.');
